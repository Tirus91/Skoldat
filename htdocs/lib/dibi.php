...
//netteloader=IDibiVariable,IDataSource,IDibiProfiler,IDibiDriver,DibiObject,DibiException,DibiDriverException,DibiConnection,DibiResult,DibiResultIterator,DibiRow,DibiTranslator,DibiVariable,DibiDataSource,DibiFluent,DibiDatabaseInfo,DibiTableInfo,DibiResultInfo,DibiColumnInfo,DibiForeignKeyInfo,DibiIndexInfo,DibiProfiler,dibi,DibiMySqlDriver,DibiMySqliDriver,DibiOdbcDriver,DibiPdoDriver,DibiPostgreDriver,DibiSqliteDriver
...
if (version_compare(PHP_VERSION, '5.1.0', '<')) {
    throw
    new
    Exception('dibi needs PHP 5.1.0 or newer.');
...
IDibiVariable {
    ...
    function
    toSql(DibiTranslator$translator, $modifier);
}
...
IDibiProfiler {
    ...
    function
    before(DibiConnection$connection, $event, $sql = NULL);

    function
    after($ticket, $result = NULL);

    function
    exception(DibiDriverException$exception);
}
...
IDebuggable {
    ...
    public
    function
    __construct($message = NULL, $code = 0, $sql = NULL) {
...
DibiObject {
    ...
    __construct($config, $name = NULL) {
        if (class_exists('Debug', FALSE)) {
            ...
            throw
            new
            InvalidArgumentException('Configuration must be array, string or ArrayObject.');
        ...
        if (!class_exists($class, FALSE)) {
            ...
            if (!class_exists($class, FALSE)) {
                throw
                new
                DibiException("Unable to create instance of dibi driver '$class'.");
            }
        }$config['name'] = $name;
        $this->config = $config;
        ...
        if (!empty($config['profiler'])) {
            ...
            if (is_numeric($class) || is_bool($class)) {
                ...
                throw
                new
                DibiException("Unable to create instance of dibi profiler '$class'.");
    ...
    static
    function
    alias(&$config, $key, $alias = NULL) {
        if (isset($config[$key]))
            return;if ($alias !== NULL && isset($config[$alias])) {
    ...
    dataSource($args) {
        ...
        return
                new
                DibiDataSource($translator->translate($args), $this);
    }
    ...
    getAffectedRows() {
        ...
        if (!is_int($rows) || $rows < 0)
            throw
            new
            DibiException('Cannot retrieve number of affected rows.');return$rows;
    }
    ...
    getInsertId($sequence = NULL) {
        ...
        if ($id < 1)
            throw
            new
            DibiException('Cannot retrieve last generated ID.');return(int) $id;
    }
    ...
    begin($savepoint = NULL) {
        ...
        if (!$savepoint && $this->inTxn) {
            throw
            new
            DibiException('There is already an active transaction.');
        }if ($this->profiler !== NULL) {
    ...
    commit($savepoint = NULL) {
        if (!$this->inTxn) {
            throw
            new
            DibiException('There is no active transaction.');
        }if ($this->profiler !== NULL) {
    ...
    public
    function
    unescape($value, $type = dibi::BINARY) {
    ...
    update($table, $args) {
        if (!(is_array($args) || $args
            ...
            throw
            new
            InvalidArgumentException('Arguments must be array or ArrayObject.');
    ...
    loadFile($file) {
        ...
        if (!$handle) {
            throw
            new
            FileNotFoundException("Cannot open file '$file'.");
        }$count = 0;
        $sql = '';
        while (!feof($handle)) {
            $s = fgets($handle);
    ...
    getDatabaseInfo() {
        ...
        return
                new
                DibiDatabaseInfo($this->driver, isset($this->config['database']) ? $this->config['database'] : NULL);
    }
    ...
    __wakeup() {
        throw
        new
        NotSupportedException('You cannot serialize or unserialize ' . $this->getClass() . ' instances.');
    }
...
IDataSource {
    ...
    getDriver() {
        if ($this->driver === NULL) {
            throw
            new
            InvalidStateException('Result-set was released from memory.');
        }return$this->driver;
    }
    ...
    getIterator($offset = NULL, $limit = NULL) {
        return
                new
                DibiResultIterator($this, $offset, $limit);
    }
    ...
    fetch() {
        if ($this->withTables === FALSE) {
            ...
            if (!is_array($row))
                return
                        FALSE;$row = array_combine($this->withTables, $row);
        }$this->fetched = TRUE;
        if ($this->xlat !== NULL) {
            foreach ($this->xlat
            as$col => $type) {
    ...
    fetchAssoc($assoc) {
        ...
        as$as) {
            if ($as !== '#' && $as !== '=' && $as !== '@' && !isset($row[$as])) {
                throw
                new
                InvalidArgumentException("Unknown column '$as' in associative descriptor.");
            }
        }$leaf = '@';
        $last = count($assoc) - 1;
    ...
    fetchPairs($key = NULL, $value = NULL) {
        ...
        if ($value === NULL) {
            if ($key !== NULL) {
                throw
                new
                InvalidArgumentException("Either none or both columns must be specified.");
            ...
            if (!isset($row[$value])) {
                throw
                new
                InvalidArgumentException("Unknown value column '$value'.");
            }if ($key === NULL) {
                do {
                    $data[] = $row[$value];
                } while ($row = $this->fetch());
                ...
                throw
                new
                InvalidArgumentException("Unknown key column '$key'.");
            }
        }do {
    ...
    getType($col) {
        return
                isset($this->xlat[$col]) ? $this->xlat[$col] : NULL;
    }
    ...
    convert($value, $type, $format = NULL) {
        if ($value === NULL || $value === FALSE) {
            ...
            case
            dibi::TEXT:return(string) $value;
            case
            dibi::BINARY:return$this->getDriver()->unescape($value, $type);
            case
            dibi::INTEGER:return(int) $value;
            case
            dibi::FLOAT:return(float) $value;
            case
            dibi::DATE:case
            dibi::DATETIME:if ($value == NULL) {
                    return
                            NULL;
                } elseif ($format === NULL) {
                    return
                            is_numeric($value) ? (int) $value : strtotime($value);
                } elseif ($format === TRUE) {
                    return
                            new
                    ...
                    return
                            date($format, $value);
                } elseif (class_exists('DateTime', FALSE)) {
                    $value = new
                            DateTime($value);
            ...
            dibi::BOOL:return((bool) $value) && $value !== 'f' && $value !== 'F';
...
Iterator, Countable {
    ...
    public
    function
    __construct(DibiResult$result, $offset = NULL, $limit = NULL) {
        $this->result = $result;
...
ArrayObject {
    ...
    asDate($key, $format = NULL) {
        ...
        if ($time == NULL) {
            ...
            return
                    new
                    DateTime(is_numeric($time) ? date('Y-m-d H:i:s', $time) : $time);
...
DibiObject {
    ...
    public
    function
    __construct(IDibiDriver$driver) {
        $this->driver = $driver;
    }
    ...
    translate(array$args) {
        ...
        while ($cursor < count($args)) {
            ...
            if (is_string($arg)) {
                ...
                if (strlen($arg) === $toSkip) {
                    ...
                    $sql[] = substr($arg, 0, $toSkip) . preg_replace_callback('/(?=[`[\'":%?])(?:`(.+?)`|\[(.+?)\]|(\')((?:\'\'|[^\'])*)\'|(")((?:""|[^"])*)"|(\'|")|:(\S*?:)([a-zA-Z0-9._]?)|%([a-zA-Z]{1,4})(?![a-zA-Z])|(\?))/s', array($this, 'cb'), substr($arg, $toSkip));
                }continue;
            }if ($comment) {
                $sql[] = '...';
                continue;
                ...
                if (is_string(key($arg))) {
                    if ($commandIns === NULL) {
                        ...
                        if ($lastArr === $cursor - 1)
                            $sql[] = ',';$sql[] = $this->formatValue($arg, $commandIns ? 'l' : 'a');
                    }$lastArr = $cursor;
                    continue;
                }elseif ($cursor === 1) {
                    $cursor = 0;
        ...
        if ($this->hasError) {
            throw
            new
            DibiException('SQL translate error', 0, $sql);
    ...
    formatValue($value, $modifier) {
        if ($value
            ...
            $vx = $kx = array();
            switch ($modifier) {
                case'and':case'or':if (empty($value)) {
                        return'1=1';
                    }foreach ($value
                    as$k => $v) {
                        if (is_string($k)) {
                ...
                case'n':foreach ($value
                    as$k => $v) {
                        if (is_string($k)) {
                            ...
                            implode(', ', $vx);
                case'a':foreach ($value
                    as$k => $v) {
                        $pair = explode('%', $k, 2);
                        $vx[] = $this->delimite($pair[0]) . '=' . $this->formatValue($v, isset($pair[1]) ? $pair[1] : (is_array($v) ? 'ex' : FALSE));
                    }return
                            implode(', ', $vx);
                case'l':foreach ($value
                    as$k => $v) {
                        $pair = explode('%', $k, 2);
                        $vx[] = $this->formatValue($v, isset($pair[1]) ? $pair[1] : (is_array($v) ? 'ex' : FALSE));
                    }return'(' . ($vx ? implode(', ', $vx) : 'NULL') . ')';
                case'v':foreach ($value
                    as$k => $v) {
                        $pair = explode('%', $k, 2);
                        $kx[] = $this->delimite($pair[0]);
                        $vx[] = $this->formatValue($v, isset($pair[1]) ? $pair[1] : (is_array($v) ? 'ex' : FALSE));
                    }return'(' . implode(', ', $kx) . ') VALUES (' . implode(', ', $vx) . ')';
                case'm':foreach ($value
                    as$k => $v) {
                        if (is_array($v)) {
                            if (isset($proto)) {
                ...
                case'by':foreach ($value
                    as$k => $v) {
                        if (is_string($k)) {
                            ...
                            implode(', ', $vx);
                case'ex':case'sql':$translator = new
                            self($this->driver);
            ...
            } elseif ($value !== NULL && !is_scalar($value) && !($value
                    instanceof
                    DateTime)) {
                $this->hasError = TRUE;
                return'**Unexpected type ' . gettype($value) . '**';
            }switch ($modifier) {
                case's':case'bin':case'b':return$value === NULL ? 'NULL' : $this->driver->escape($value, $modifier);
                case'sn':return$value == '' ? 'NULL' : $this->driver->escape($value, dibi::TEXT);
                case'in':if ($value == '')
                        $value = NULL;case'i':case'u':if (is_string($value) && preg_match('#[+-]?\d+(e\d+)?$#A', $value)) {
                        return$value;
                    } else {
                        return$value === NULL ? 'NULL' : (string) (int) ($value + 0);
                    }case'f':if (is_string($value) && is_numeric($value) && strpos($value, 'x') === FALSE) {
                        return$value;
                    } else {
                        return$value === NULL ? 'NULL' : rtrim(rtrim(number_format($value, 5, '.', ''), '0'), '.');
                    }case'd':case't':if ($value === NULL) {
                        return'NULL';
                    } else {
                        if (is_numeric($value)) {
                            $value = (int) $value;
                        } elseif (is_string($value)) {
                            $value = class_exists('DateTime', FALSE) ? new
                                    DateTime($value) : strtotime($value);
                        }return$this->driver->escape($value, $modifier);
                    }case'by':case'n':return$this->delimite($value);
                case'ex':case'sql':$value = (string) $value;
                    $toSkip = strcspn($value, '`[\'":');
                    if (strlen($value) === $toSkip) {
                        return$value;
                    } else {
                        return
                                substr($value, 0, $toSkip) . preg_replace_callback('/(?=[`[\'":])(?:`(.+?)`|\[(.+?)\]|(\')((?:\'\'|[^\'])*)\'|(")((?:""|[^"])*)"|(\'|")|:(\S*?:)([a-zA-Z0-9._]?))/s', array($this, 'cb'), substr($value, $toSkip));
                    }case'SQL':return(string) $value;
                case'and':case'or':case'a':case'l':case'v':$this->hasError = TRUE;
                    return'**Unexpected type ' . gettype($value) . '**';
                default:$this->hasError = TRUE;
                    return"**Unknown or invalid modifier %$modifier**";
            }
        }if (is_string($value))
            return$this->driver->escape($value, dibi::TEXT);if (is_int($value) || is_float($value))
            return
                    rtrim(rtrim(number_format($value, 5, '.', ''), '0'), '.');if (is_bool($value))
            return$this->driver->escape($value, dibi::BOOL);if ($value === NULL)
            return'NULL';if ($value
                instanceof
                IDibiVariable)
            return$value->toSql($this, NULL);if ($value
                instanceof
                DateTime)
            return$this->driver->escape($value, dibi::DATETIME);$this->hasError = TRUE;
        ...
        if (!empty($matches[11])) {
            ...
            if ($cursor >= count($this->args) && $mod !== 'else' && $mod !== 'end') {
                ...
                if ($this->args[$cursor] !== NULL)
                    $this->limit = (int) $this->args[$cursor];$cursor++;
                return'';
            }elseif ($mod === 'ofs') {
                if ($this->args[$cursor] !== NULL)
                    $this->offset = (int) $this->args[$cursor];$cursor++;
                return'';
            }else {
                $cursor++;
            ...
            return'...';if ($matches[1])
            return$this->delimite($matches[1]);if ($matches[2])
            return$this->delimite($matches[2]);if ($matches[3])
            return$this->driver->escape(str_replace("''", "'", $matches[4]), dibi::TEXT);if ($matches[5])
            return$this->driver->escape(str_replace('""', '"', $matches[6]), dibi::TEXT);if ($matches[7]) {
            $this->hasError = TRUE;
            return'**Alone quote**';
        }if ($matches[8]) {
    ...
    subCb($m) {
        $m = $m[1];
        return
                isset(dibi::$substs[$m]) ? dibi::$substs[$m] : call_user_func(dibi::$substFallBack, $m);
    }

}

class
DibiVariable extends
DibiObject implements
IDibiVariable {
...
IDataSource {
    ...
    public
    function
    __construct($sql, DibiConnection$connection) {
        if (strpos($sql, ' ') === FALSE) {
    ...
    toDataSource() {
        return
                new
                self($this->__toString(), $this->connection);
    }
    ...
    count() {
        if ($this->count === NULL) {
            $this->count = $this->conds || $this->offset || $this->limit ? (int) $this->connection->nativeQuery('SELECT COUNT(*) FROM (' . $this->__toString() . ') AS t')->fetchSingle() : $this->getTotalCount();
        }return$this->count;
    }
...
IDataSource {
    ...
    public
    static$masks = array('SELECT' => array('SELECT', 'DISTINCT', 'FROM', 'WHERE', 'GROUP BY', 'HAVING', 'ORDER BY', 'LIMIT', 'OFFSET'), 'UPDATE' => array('UPDATE', 'SET', 'WHERE', 'ORDER BY', 'LIMIT'), 'INSERT' => array('INSERT', 'INTO', 'VALUES', 'SELECT'), 'DELETE' => array('DELETE', 'FROM', 'USING', 'WHERE', 'ORDER BY', 'LIMIT'));
    public
    static$modifiers = array('SELECT' => '%n', 'FROM' => '%n', 'IN' => '%l', 'VALUES' => '%l', 'SET' => '%a', 'WHERE' => '%and', 'HAVING' => '%and', 'ORDER BY' => '%by', 'GROUP BY' => '%by');
    public
    static$separators = array('SELECT' => ',', 'FROM' => FALSE, 'WHERE' => 'AND', 'GROUP BY' => ',', 'HAVING' => 'AND', 'ORDER BY' => ',', 'LIMIT' => FALSE, 'OFFSET' => FALSE, 'SET' => ',', 'VALUES' => ',', 'INTO' => FALSE);
    private$connection;
    private$command;
    ...
    __call($clause, $args) {
        ...
        if ($this->command === NULL) {
            ...
            if ($arg === TRUE) {
                $args = array();
            } elseif (is_string($arg) && preg_match('#^[a-z:_][a-z0-9_.:]*$#i', $arg)) {
                $args = array('%n', $arg);
            } elseif ($arg
                    instanceof
                    self) {
    ...
    getFlag($flag) {
        return
                isset($this->flags[strtoupper($flag)]);
    }
    ...
    toDataSource() {
        return
                new
                DibiDataSource($this->connection->sql($this->_export()), $this->connection);
    }
    ...
    protected
    function
    _export($clause = NULL, $args = array()) {
        if ($clause === NULL) {
            $data = $this->clauses;
        } else {
            $clause = self::_formatClause($clause);
    ...
    _formatClause($s) {
        if ($s === 'order' || $s === 'group') {
            ...
            trigger_error("Did you mean '$s'?", E_USER_NOTICE);
                ...
                strtoupper(preg_replace('#[A-Z]#', ' $0', $s));
    }
...
DibiObject {
    ...
    getTable($name) {
        ...
        if (isset($this->tables[$l])) {
            ...
            throw
            new
            DibiException("Database '$this->name' has no table '$name'.");
        }
    }
    ...
    hasTable($name) {
        $this->init();
        return
                isset($this->tables[strtolower($name)]);
    }
    ...
    init() {
        if ($this->tables === NULL) {
            ...
            foreach ($this->driver->getTables()as$info) {
                $this->tables[strtolower($info['name'])] = new
                        DibiTableInfo($this->driver, $info);
            }
        }
    }
...
DibiObject {
    ...
    public
    function
    __construct(IDibiDriver$driver, array$info) {
        $this->driver = $driver;
        $this->name = $info['name'];
    ...
    getColumn($name) {
        ...
        if (isset($this->columns[$l])) {
            ...
            throw
            new
            DibiException("Table '$this->name' has no column '$name'.");
        }
    }
    ...
    hasColumn($name) {
        $this->initColumns();
        return
                isset($this->columns[strtolower($name)]);
    }
    ...
    initColumns() {
        if ($this->columns === NULL) {
            ...
            foreach ($this->driver->getColumns($this->name)as$info) {
                $this->columns[strtolower($info['name'])] = new
                        DibiColumnInfo($this->driver, $info);
            }
        }
    }
    ...
    initIndexes() {
        if ($this->indexes === NULL) {
            ...
            foreach ($this->driver->getIndexes($this->name)as$info) {
                ...
                }$this->indexes[strtolower($info['name'])] = new
                        DibiIndexInfo($info);
...
DibiObject {
    ...
    getColumn($name) {
        ...
        if (isset($this->names[$l])) {
            ...
            throw
            new
            DibiException("Result set has no column '$name'.");
        }
    }
    ...
    hasColumn($name) {
        $this->initColumns();
        return
                isset($this->names[strtolower($name)]);
    }
    ...
    initColumns() {
        if ($this->columns === NULL) {
            ...
            foreach ($this->driver->getColumnsMeta()as$info) {
                $this->columns[] = $this->names[$info['name']] = new
                        DibiColumnInfo($this->driver, $info);
            }
        }
    }
...
DibiObject {
    ...
    getTable() {
        if (empty($this->info['table'])) {
            throw
            new
            DibiException("Table name is unknown.");
        }return
                new
                DibiTableInfo($this->driver, array('name' => $this->info['table']));
    }
    ...
    getSize() {
        return
                isset($this->info['size']) ? (int) $this->info['size'] : NULL;
    }
    ...
    isNullable() {
        return
                isset($this->info['nullable']) ? (bool) $this->info['nullable'] : NULL;
    }
    ...
    isAutoIncrement() {
        return
                isset($this->info['autoincrement']) ? (bool) $this->info['autoincrement'] : NULL;
    }
    ...
    getDefault() {
        return
                isset($this->info['default']) ? $this->info['default'] : NULL;
    }
    ...
    getVendorInfo($key) {
        return
                isset($this->info['vendor'][$key]) ? $this->info['vendor'][$key] : NULL;
    }
    ...
    function
    detectType($type) {
        static$patterns = array('BYTEA|BLOB|BIN' => dibi::BINARY, 'TEXT|CHAR' => dibi::TEXT, 'BYTE|COUNTER|SERIAL|INT|LONG' => dibi::INTEGER, 'CURRENCY|REAL|MONEY|FLOAT|DOUBLE|DECIMAL|NUMERIC|NUMBER' => dibi::FLOAT, '^TIME$' => dibi::TIME, 'TIME' => dibi::DATETIME, 'YEAR|DATE' => dibi::DATE, 'BOOL|BIT' => dibi::BOOL);
...
DibiObject {
    ...
    public
    function
    __construct($name, array$references) {
        $this->name = $name;
...
IDibiProfiler {
    ...
    public
    static$table = array(array('Time', 'SQL Statement', 'Rows', 'Connection'));
        ...
        if (($event & $this->filter) === 0)
            ...
            try {
                ...
                if (count(self::$table) < self::FIREBUG_MAX_ROWS) {
                    self::$table[] = array(sprintf('%0.3f', dibi::$elapsedTime * 1000), strlen($sql) > self::FIREBUG_MAX_LENGTH ? substr($sql, 0, self::FIREBUG_MAX_LENGTH) . '...' : $sql, $count, $connection->getConfig('driver') . '/' . $connection->getConfig('name'));
                ...
                $payload = array(array('Type' => 'TABLE', 'Label' => 'dibi profiler (' . dibi::$numOfQueries . ' SQL queries took ' . sprintf('%0.3f', dibi::$totalTime * 1000) . ' ms)'), self::$table);
    ...
    function
    exception(DibiDriverException$exception) {
        if ((self::EXCEPTION & $this->filter) === 0)
            return;if ($this->useFirebug) {
    ...
    json_encode($val) {
        if (is_array($val) && (!$val || array_keys($val) === range(0, count($val) - 1))) {
            ...
            $val = str_replace(array("\\", "\x00"), array("\\\\", "\\u0000"), $val);
            return'"' . addcslashes($val, "\x8\x9\xA\xC\xD/\"") . '"';
        }if (is_int($val) || is_float($val)) {
            return
                    rtrim(rtrim(number_format($val, 5, '.', ''), '0'), '.');
        }if (is_bool($val)) {
...
dibi {
    ...
    const
            ...
            VERSION = '1.2';
            const
            REVISION = '3b2ca19 released on 2009-09-18';
            const
    ...
    static$substs = array();
    public
    static$substFallBack = array(__CLASS__, 'defaultSubstFallback');
    private
    ...
    static$totalTime;
    public
    static$numOfQueries = 0;
    public
    static$defaultDriver = 'mysql';
    ...
    static
    function
    connect($config = array(), $name = 0) {
        return
                self::$connection = self::$registry[$name] = new
                DibiConnection($config, $name);
    }
    ...
    getConnection($name = NULL) {
        if ($name === NULL) {
            if (self::$connection === NULL) {
                throw
                new
                DibiException('Dibi is not connected to database.');
            }return
                    self::$connection;
        }if (!isset(self::$registry[$name])) {
            throw
            new
            DibiException("There is no connection named '$name'.");
        }return
                self::$registry[$name];
    }
    ...
    getProfiler() {
        return
                self::getConnection()->getProfiler();
    }
    ...
    dataSource($args) {
        $args = func_get_args();
        return
                self::getConnection()->dataSource($args);
    }
    ...
    fetch($args) {
        $args = func_get_args();
        return
                self::getConnection()->query($args)->fetch();
    }
    ...
    fetchAll($args) {
        $args = func_get_args();
        return
                self::getConnection()->query($args)->fetchAll();
    }
    ...
    fetchSingle($args) {
        $args = func_get_args();
        return
                self::getConnection()->query($args)->fetchSingle();
    }
    ...
    fetchPairs($args) {
        $args = func_get_args();
        return
                self::getConnection()->query($args)->fetchPairs();
    }
    ...
    getDatabaseInfo() {
        return
                self::getConnection()->getDatabaseInfo();
    }
    ...
    loadFile($file) {
        return
                self::getConnection()->loadFile($file);
    }
    ...
    __callStatic($name, $args) {
        return
                call_user_func_array(array(self::getConnection(), $name), $args);
    }
    ...
    select($args) {
        $args = func_get_args();
        return
                call_user_func_array(array(self::getConnection(), 'select'), $args);
    }
    ...
    update($table, $args) {
        return
                self::getConnection()->update($table, $args);
    }
    ...
    insert($table, $args) {
        return
                self::getConnection()->insert($table, $args);
    }
    ...
    delete($table) {
        return
                self::getConnection()->delete($table);
    }
    ...
    datetime($time = NULL) {
        if ($time === NULL) {
            ...
            $time = class_exists('DateTime', FALSE) ? new
                    DateTime($time) : strtotime($time);
        }return
                new
                DibiVariable($time, dibi::DATETIME);
    }
    ...
    static
    function
    defaultSubstFallback($expr) {
        throw
        new
        InvalidStateException("Missing substitution for '$expr' expression.");
    }
    ...
    dump($sql = NULL, $return = FALSE) {
        ...
        if ($sql
            ...
            if ($sql === NULL)
                $sql = self::$sql;static$keywords1 = 'SELECT|UPDATE|INSERT(?:\s+INTO)?|REPLACE(?:\s+INTO)?|DELETE|FROM|WHERE|HAVING|GROUP\s+BY|ORDER\s+BY|LIMIT|SET|VALUES|LEFT\s+JOIN|INNER\s+JOIN|TRUNCATE';
    ...
    static
    function
    highlightCallback($matches) {
        if (!empty($matches[1]))
            return'<em style="color:gray">' . $matches[1] . '</em>';if (!empty($matches[2]))
            return'<strong style="color:red">' . $matches[2] . '</strong>';if (!empty($matches[3]))
            return'<strong style="color:blue">' . $matches[3] . '</strong>';if (!empty($matches[4]))
    ...
    getColophon($sender = NULL) {
        $arr = array('Number of SQL queries: ' . dibi::$numOfQueries . (dibi::$totalTime === NULL ? '' : ', elapsed time: ' . sprintf('%0.3f', dibi::$totalTime * 1000) . ' ms'));
...
IDibiDriver {
    ...
    __construct() {
        if (!extension_loaded('mysql')) {
            throw
            new
            DibiDriverException("PHP extension 'mysql' is not loaded.");
        }
    }
    ...
    connect(array&$config) {
        ...
        if (isset($config['resource'])) {
            ...
            if (!isset($config['username']))
                $config['username'] = ini_get('mysql.default_user');if (!isset($config['password']))
                $config['password'] = ini_get('mysql.default_password');if (!isset($config['host'])) {
                ...
                if ($host) {
                    ...
                    if (!isset($config['socket']))
                        $config['socket'] = ini_get('mysql.default_socket');$config['host'] = NULL;
                }
            ...
            throw
            new
            DibiDriverException(mysql_error(), mysql_errno());
            ...
            if (function_exists('mysql_set_charset')) {
                ...
                if (!$ok) {
                    throw
                    new
                    DibiDriverException(mysql_error($this->connection), mysql_errno($this->connection));
                }
            }
    ...
    query($sql) {
        if ($this->buffered) {
            ...
            DibiDriverException(mysql_error($this->connection), mysql_errno($this->connection), $sql);
                ...
                is_resource($this->resultSet) ? clone$this : NULL;
    }
    ...
    escape($value, $type) {
        switch ($type) {
            case
            dibi::TEXT:return"'" . mysql_real_escape_string($value, $this->connection) . "'";
            case
            dibi::BINARY:return"_binary'" . mysql_real_escape_string($value, $this->connection) . "'";
            case
            dibi::IDENTIFIER:$value = str_replace('`', '``', $value);
                return'`' . str_replace('.', '`.`', $value) . '`';
            case
            dibi::BOOL:return$value ? 1 : 0;
            case
            dibi::DATE:return$value
                        instanceof
                        DateTime ? $value->format("'Y-m-d'") : date("'Y-m-d'", $value);
            case
            dibi::DATETIME:return$value
                        instanceof
                        DateTime ? $value->format("'Y-m-d H:i:s'") : date("'Y-m-d H:i:s'", $value);
            default:throw
                new
                InvalidArgumentException('Unsupported type.');
        }
    }
    ...
    applyLimit(&$sql, $limit, $offset) {
        if ($limit < 0 && $offset < 1)
            return;$sql.=' LIMIT ' . ($limit < 0 ? '18446744073709551615' : (int) $limit) . ($offset > 0 ? ' OFFSET ' . (int) $offset : '');
    }
    ...
    getRowCount() {
        if (!$this->buffered) {
            throw
            new
            DibiDriverException('Row count is not available for unbuffered queries.');
        }return
    ...
    fetch($assoc) {
        return
                mysql_fetch_array($this->resultSet, $assoc ? MYSQL_ASSOC : MYSQL_NUM);
    }
    ...
    seek($row) {
        if (!$this->buffered) {
            throw
            new
            DibiDriverException('Cannot seek an unbuffered result set.');
        }return
    ...
    getColumnsMeta() {
        ...
        for ($i = 0; $i < $count; $i++) {
            ...
            $res[] = array('name' => $row['name'], 'table' => $row['table'], 'fullname' => $row['table'] ? $row['table'] . '.' . $row['name'] : $row['name'], 'nativetype' => strtoupper($row['type']), 'vendor' => $row);
        }return$res;
    }
    ...
    getTables() {
        ...
        while ($row = $this->fetch(FALSE)) {
            $res[] = array('name' => $row[0], 'view' => isset($row[1]) && $row[1] === 'VIEW');
        }$this->free();
        return$res;
    }
    ...
    getColumns($table) {
        ...
        while ($row = $this->fetch(TRUE)) {
            ...
            $res[] = array('name' => $row['Field'], 'table' => $table, 'nativetype' => strtoupper($type[0]), 'size' => isset($type[1]) ? (int) $type[1] : NULL, 'nullable' => $row['Null'] === 'YES', 'default' => $row['Default'], 'autoincrement' => $row['Extra'] === 'auto_increment', 'vendor' => $row);
        }$this->free();
        return$res;
    }
...
IDibiDriver {
    ...
    __construct() {
        if (!extension_loaded('mysqli')) {
            throw
            new
            DibiDriverException("PHP extension 'mysqli' is not loaded.");
        }
    }
    ...
    connect(array&$config) {
        ...
        if (isset($config['resource'])) {
            ...
            if (!isset($config['username']))
                $config['username'] = ini_get('mysqli.default_user');if (!isset($config['password']))
                $config['password'] = ini_get('mysqli.default_pw');if (!isset($config['socket']))
                $config['socket'] = ini_get('mysqli.default_socket');if (!isset($config['port']))
                $config['port'] = NULL;if (!isset($config['host'])) {
            ...
            if ($errno = mysqli_connect_errno()) {
                throw
                new
                DibiDriverException(mysqli_connect_error(), $errno);
            }
        }if (isset($config['charset'])) {
            $ok = FALSE;
            if (version_compare(PHP_VERSION, '5.1.5', '>=')) {
                ...
                if (!$ok) {
                    throw
                    new
                    DibiDriverException(mysqli_error($this->connection), mysqli_errno($this->connection));
                }
            }
        }if (isset($config['sqlmode'])) {
    ...
    query($sql) {
        ...
        if (mysqli_errno($this->connection)) {
            throw
            new
            DibiDriverException(mysqli_error($this->connection), mysqli_errno($this->connection), $sql);
        }return
                is_object($this->resultSet) ? clone$this : NULL;
    }
    ...
    escape($value, $type) {
        switch ($type) {
            case
            dibi::TEXT:return"'" . mysqli_real_escape_string($this->connection, $value) . "'";
            case
            dibi::BINARY:return"_binary'" . mysqli_real_escape_string($this->connection, $value) . "'";
            case
            dibi::IDENTIFIER:$value = str_replace('`', '``', $value);
                return'`' . str_replace('.', '`.`', $value) . '`';
            case
    ...
    getRowCount() {
        if (!$this->buffered) {
            ...
            DibiDriverException('Row count is not available for unbuffered queries.');
                ...
                mysqli_num_rows($this->resultSet);
    }
    ...
    fetch($assoc) {
        return
                mysqli_fetch_array($this->resultSet, $assoc ? MYSQLI_ASSOC : MYSQLI_NUM);
    }
    ...
    getColumnsMeta() {
        ...
        for ($i = 0; $i < $count; $i++) {
            ...
            $res[] = array('name' => $row['name'], 'table' => $row['orgtable'], 'fullname' => $row['table'] ? $row['table'] . '.' . $row['name'] : $row['name'], 'nativetype' => $types[$row['type']], 'vendor' => $row);
        }return$res;
    }
...
IDibiDriver {
    ...
    __construct() {
        if (!extension_loaded('odbc')) {
            throw
            new
            DibiDriverException("PHP extension 'odbc' is not loaded.");
        }
    }
    ...
    connect(array&$config) {
        if (isset($config['resource'])) {
            ...
            if (!isset($config['username']))
                $config['username'] = ini_get('odbc.default_user');if (!isset($config['password']))
                $config['password'] = ini_get('odbc.default_pw');if (!isset($config['dsn']))
                $config['dsn'] = ini_get('odbc.default_db');if (empty($config['persistent'])) {
            ...
            throw
            new
            DibiDriverException(odbc_errormsg() . ' ' . odbc_error());
        }
    }
    ...
    query($sql) {
        ...
        if ($this->resultSet === FALSE) {
            throw
            new
            DibiDriverException(odbc_errormsg($this->connection) . ' ' . odbc_error($this->connection), 0, $sql);
        }return
                is_resource($this->resultSet) ? clone$this : NULL;
    }
    ...
    getInsertId($sequence) {
        throw
        new
        NotSupportedException('ODBC does not support autoincrementing.');
    }
    ...
    begin($savepoint = NULL) {
        if (!odbc_autocommit($this->connection, FALSE)) {
            throw
            new
            DibiDriverException(odbc_errormsg($this->connection) . ' ' . odbc_error($this->connection));
        }
    }
    ...
    escape($value, $type) {
        switch ($type) {
            case
            dibi::TEXT:case
            dibi::BINARY:return"'" . str_replace("'", "''", $value) . "'";
            case
            dibi::IDENTIFIER:$value = str_replace(array('[', ']'), array('[[', ']]'), $value);
                return'[' . str_replace('.', '].[', $value) . ']';
            case
            dibi::BOOL:return$value ? 1 : 0;
            case
            dibi::DATE:return$value
                        instanceof
                        DateTime ? $value->format("#m/d/Y#") : date("#m/d/Y#", $value);
            case
            dibi::DATETIME:return$value
                        instanceof
                        DateTime ? $value->format("#m/d/Y H:i:s#") : date("#m/d/Y H:i:s#", $value);
            default:throw
                new
                InvalidArgumentException('Unsupported type.');
        }
    ...
    applyLimit(&$sql, $limit, $offset) {
        if ($limit >= 0) {
            ...
            throw
            new
            InvalidArgumentException('Offset is not implemented in driver odbc.');
    }
    ...
    fetch($assoc) {
        if ($assoc) {
            return
                    odbc_fetch_array($this->resultSet, ++$this->row);
        } else {
            $set = $this->resultSet;
            if (!odbc_fetch_row($set, ++$this->row))
                return
                        FALSE;$count = odbc_num_fields($set);
            $cols = array();
            for ($i = 1; $i <= $count; $i++)
                $cols[] = odbc_result($set, $i);return$cols;
        }
    }
...
IDibiDriver {
    ...
    __construct() {
        if (!extension_loaded('pdo')) {
            throw
            new
            DibiDriverException("PHP extension 'pdo' is not loaded.");
        }
    }
    ...
    connect(array&$config) {
        ...
        DibiConnection::alias($config, 'options');
        if ($config['resource']instanceof
                PDO) {
            $this->connection = $config['resource'];
        }else
            try {
                $this->connection = new
                        PDO($config['dsn'], $config['username'], $config['password'], $config['options']);
            } catch (PDOException$e) {
                throw
                new
                DibiDriverException($e->getMessage(), $e->getCode());
            }if (!$this->connection) {
            throw
            new
            DibiDriverException('Connecting error.');
        }
    }
    ...
    query($sql) {
        ...
        if (isset($list[$cmd])) {
            ...
            if ($this->affectedRows === FALSE) {
                ...
                throw
                new
                DibiDriverException("SQLSTATE[$err[0]]: $err[2]", $err[1], $sql);
            }return
                    NULL;
        } else {
    ...
    begin($savepoint = NULL) {
        if (!$this->connection->beginTransaction()) {
            ...
            throw
            new
            DibiDriverException("SQLSTATE[$err[0]]: $err[2]", $err[1]);
        }
    }
    ...
    escape($value, $type) {
        switch ($type) {
            case
            dibi::TEXT:return$this->connection->quote($value, PDO::PARAM_STR);
            case
            dibi::BINARY:return$this->connection->quote($value, PDO::PARAM_LOB);
            case
            dibi::IDENTIFIER:switch ($this->connection->getAttribute(PDO::ATTR_DRIVER_NAME)) {
                    case'mysql':$value = str_replace('`', '``', $value);
                        return'`' . str_replace('.', '`.`', $value) . '`';
                    case'pgsql':$a = strrpos($value, '.');
                        if ($a === FALSE) {
                            ...
                            return
                                    substr($value, 0, $a) . '."' . str_replace('"', '""', substr($value, $a + 1)) . '"';
                        }case'sqlite':case'sqlite2':$value = strtr($value, '[]', '  ');
                    case'odbc':case'oci':case'mssql':$value = str_replace(array('[', ']'), array('[[', ']]'), $value);
                        return'[' . str_replace('.', '].[', $value) . ']';
                    default:return$value;
                }case
            dibi::BOOL:return$this->connection->quote($value, PDO::PARAM_BOOL);
            case
            dibi::DATE:return$value
                        instanceof
                        DateTime ? $value->format("'Y-m-d'") : date("'Y-m-d'", $value);
    ...
    applyLimit(&$sql, $limit, $offset) {
        if ($limit < 0 && $offset < 1)
            return;switch ($this->connection->getAttribute(PDO::ATTR_DRIVER_NAME)) {
            case'mysql':$sql.=' LIMIT ' . ($limit < 0 ? '18446744073709551615' : (int) $limit) . ($offset > 0 ? ' OFFSET ' . (int) $offset : '');
                break;
            case'pgsql':if ($limit >= 0)
                    $sql.=' LIMIT ' . (int) $limit;if ($offset > 0)
                    $sql.=' OFFSET ' . (int) $offset;break;
            case'sqlite':case'sqlite2':$sql.=' LIMIT ' . $limit . ($offset > 0 ? ' OFFSET ' . (int) $offset : '');
                break;
            case'oci':if ($offset > 0) {
            ...
            case'odbc':case'mssql':if ($offset < 1) {
                    $sql = 'SELECT TOP ' . (int) $limit . ' * FROM (' . $sql . ')';
                    break;
                }default:throw
                new
                NotSupportedException('PDO or driver does not support applying limit or offset.');
        }
    }
    ...
    getRowCount() {
        throw
        new
        NotSupportedException('Row count is not available for unbuffered queries.');
    }
    ...
    seek($row) {
        throw
        new
        NotSupportedException('Cannot seek an unbuffered result set.');
    }
    ...
    getColumnsMeta() {
        ...
        $res = array();
        for ($i = 0; $i < $count; $i++) {
            $row = @$this->resultSet->getColumnMeta($i);
            if ($row === FALSE) {
                throw
                new
                DibiDriverException('Driver does not support meta data.');
            }$res[] = array('name' => $row['name'], 'table' => $row['table'], 'nativetype' => $row['native_type'], 'fullname' => $row['table'] ? $row['table'] . '.' . $row['name'] : $row['name'], 'vendor' => $row);
        }return$res;
    }
...
IDibiDriver {
    ...
    __construct() {
        if (!extension_loaded('pgsql')) {
            throw
            new
            DibiDriverException("PHP extension 'pgsql' is not loaded.");
        }
    }
    ...
    query($sql) {
        ...
        if ($this->resultSet === FALSE) {
            throw
            new
            DibiDriverException(pg_last_error($this->connection), 0, $sql);
        }return
                is_resource($this->resultSet) && pg_num_fields($this->resultSet) ? clone$this : NULL;
    }
    ...
    getAffectedRows() {
        return
                pg_affected_rows($this->resultSet);
    }
    ...
    getInsertId($sequence) {
        if ($sequence === NULL) {
            ...
            return
                    FALSE;$row = $this->fetch(FALSE);
        $this->free();
        return
                is_array($row) ? $row[0] : FALSE;
    }
    ...
    escape($value, $type) {
        switch ($type) {
            case
            dibi::TEXT:if ($this->escMethod) {
                    return"'" . pg_escape_string($this->connection, $value) . "'";
                } else {
                    return"'" . pg_escape_string($value) . "'";
                }case
            dibi::BINARY:if ($this->escMethod) {
                    return"'" . pg_escape_bytea($this->connection, $value) . "'";
                } else {
                    return"'" . pg_escape_bytea($value) . "'";
                }case
            dibi::IDENTIFIER:$a = strrpos($value, '.');
                if ($a === FALSE) {
                    return'"' . str_replace('"', '""', $value) . '"';
                } else {
                    return
            ...
            dibi::BOOL:return$value ? 'TRUE' : 'FALSE';
            case
            dibi::DATE:return$value
                        instanceof
                        DateTime ? $value->format("'Y-m-d'") : date("'Y-m-d'", $value);
    ...
    fetch($assoc) {
        return
                pg_fetch_array($this->resultSet, NULL, $assoc ? PGSQL_ASSOC : PGSQL_NUM);
    }
    ...
    seek($row) {
        return
                pg_result_seek($this->resultSet, $row);
    }
    ...
    getTables() {
        ...
        if ($version['server'] < 8) {
            throw
            new
            DibiDriverException('Reflection requires PostgreSQL 8.');
        }$this->query("
    ...
    getColumns($table) {
        ...
        while ($row = $this->fetch(TRUE)) {
            ...
            $res[] = array('name' => $row['column_name'], 'table' => $table, 'nativetype' => strtoupper($row['udt_name']), 'size' => $size ? $size : NULL, 'nullable' => $row['is_nullable'] === 'YES', 'default' => $row['column_default'], 'autoincrement' => (int) $row['ordinal_position'] === $primary && substr($row['column_default'], 0, 7) === 'nextval', 'vendor' => $row);
        }$this->free();
        return$res;
    }
...
IDibiDriver {
    ...
    query($sql) {
        ...
        if ($this->buffered) {
            ...
            throw
            new
            DibiDriverException($msg, sqlite_last_error($this->connection), $sql);
        }return
                is_resource($this->resultSet) ? clone$this : NULL;
    }
    ...
    escape($value, $type) {
        switch ($type) {
            case
            dibi::TEXT:case
            dibi::BINARY:return"'" . sqlite_escape_string($value) . "'";
            case
            dibi::IDENTIFIER:return'[' . str_replace('.', '].[', strtr($value, '[]', '  ')) . ']';
            case
            dibi::BOOL:return$value ? 1 : 0;
            case
            dibi::DATE:return$value
                        instanceof
                        DateTime ? $value->format($this->fmtDate) : date($this->fmtDate, $value);
            case
            dibi::DATETIME:return$value
                        instanceof
                        DateTime ? $value->format($this->fmtDateTime) : date($this->fmtDateTime, $value);
            default:throw
                new
                InvalidArgumentException('Unsupported type.');
        }
    ...
    applyLimit(&$sql, $limit, $offset) {
        if ($limit < 0 && $offset < 1)
            return;$sql.=' LIMIT ' . $limit . ($offset > 0 ? ' OFFSET ' . (int) $offset : '');
    }
    ...
    getRowCount() {
        if (!$this->buffered) {
            ...
            DibiDriverException('Row count is not available for unbuffered queries.');
                ...
                sqlite_num_rows($this->resultSet);
    }
    ...
    getColumnsMeta() {
        ...
        for ($i = 0; $i < $count; $i++) {
            ...
            $pair = explode('.', $name);
            $res[] = array('name' => isset($pair[1]) ? $pair[1] : $pair[0], 'table' => isset($pair[1]) ? $pair[0] : NULL, 'fullname' => $name, 'nativetype' => NULL);
        }return$res;
    }
    ...
    public
    function
    registerFunction($name, $callback, $numArgs = -1) {
        sqlite_create_function($this->connection, $name, $callback, $numArgs);
    }

    public
    function
    registerAggregateFunction($name, $rowCallback, $agrCallback, $numArgs = -1) {
